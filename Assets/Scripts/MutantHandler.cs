...
{
    #region Variables
    //Variables for mutant character
    private NavMeshAgent _mutantAgent;
    ...
    //Variable to hold our current state, start by default
    private string _mutantState = "Start";
    #endregion
    void Start()
    {
        ...
        _mutantAgent = GetComponent<NavMeshAgent>();
    ...
    void SelectState(string state)
    {
        switch (state)
        {
            case "Start":
                ...
                _mutantAgent.SetDestination(key.transform.position);
                StartCoroutine(MutantStart());
                break;
            case "Prisoner":
                //Set destination to location of main drop gate
                _mutantAgent.SetDestination(_escapeGate.transform.position);
                StartCoroutine(FreePrisoner());
                break;
            case "Raid":
                //Set destination to drop gate guarding crypt
                _mutantAgent.SetDestination(_raidGate.transform.position);
    ...
    IEnumerator FreePrisoner()
    {
        ...
        while (_mutantState == "Prisoner")
        {
            ...
            if (Vector3.Distance(transform.position, _escapeGate.transform.position) < 0.05f)
            {
                ...
                _mutantAgent.SetDestination(_releaseGate.transform.position);
            }
            ...
            if (Vector3.Distance(transform.position, _releaseGate.transform.position) < 0.05f)
            {
                ...
                _obstacles[2].carving = false;
                _obstacles[3].carving = false;
                yield return new WaitForSecondsRealtime(1f);
                humanHandler.SelectState("Investigate");
                ogreHandler.SelectState("Heist");
                _mutantAgent.SetDestination(_cellWait.transform.position);
            }
            ...
            if (Vector3.Distance(transform.position, _cellWait.transform.position) < 0.05f)
            {
                ...
                _obstacles[0].carving = false;
                _obstacles[1].carving = false;
    ...
    IEnumerator StealTreasure()
    {
        ...
        while (_mutantState == "Raid")
        {
            ...
            if (Vector3.Distance(transform.position, _raidGate.transform.position) < 0.5f)
            {
                ...
                yield return new WaitForSecondsRealtime(2f);
                _mutantAgent.SetDestination(_cryptRaid.transform.position);
            }
    ...
    IEnumerator Escape()
    {
        ...
        _mutantAgent.SetDestination(_finalDestination.transform.position);
        yield return null;
    }
    #endregion
